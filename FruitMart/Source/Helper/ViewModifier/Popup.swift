    ...
    private var popupContent: some View {
        GeometryReader {
            VStack {self.message}
                ...
                .position(x: self.size?.width ?? $0.size.width / 2,
                          y: self.size?.height ?? $0.size.height / 2)
        }
    }
...
extension View {
    func popup<Content: View>(
        ...
        if isPresented.wrappedValue { // binding 타입 접근 가능하도록
    ...
    func popupOverContext<Item: Identifiable, Context: View>(
        item: Binding<Item?>,
        size: CGSize? = nil,
        style: PopupStyle = .none,
        ignoringEdges edges: Edge.Set = .all,
        @ViewBuilder content: (Item) -> Context
    ) -> some View {
